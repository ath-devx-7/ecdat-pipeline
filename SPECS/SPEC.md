...
| data_lifetime_years | int | X in Mosca — the scan-wide value, and the fallback for every file with no lifetime of its own. Set on the approval screen; a `probe_only` scan has no approval screen and sets it at intake |
...
| data_lifetime_years | int, null | X for this file alone, set on the approval screen. Null means no override — scored at the scan's value |
...
y_years_default: 1        # fallback migration duration
staleness_warning_days: 180

# Y per migration effort class. Assumptions, not measurements — stated in the
# pack so an operator can edit them, rather than hidden in the scorer.
y_years_by_action_class:
  config: 1
  library_upgrade: 2
  code_change: 3
  hardware: 5
...
`y_years_by_action_class` is optional but validated: every key must be a valid `ActionClass`
and every value a non-negative integer, or the pack is refused at load. A typo would
silently score that effort class at the default, which is worse than never varying — the
row would claim an assumption the pack does not make.
...
- **X** = the finding's own file's `scan_files.data_lifetime_years`, else
  `scan.data_lifetime_years` — both supplied with the approval, not at intake
- **Y** = `y_years_by_action_class[action_class]` from policy, where the action class is the
  cheapest matching `pqc_targets` rule, else `y_years_default`
- **Z** = `z_years_default` from policy, **exposed as a UI slider** on the results screen

Positive means overdue. Store all three inputs plus the result on every row.

Two of the three are assumptions rather than measurements, so each records where it came
from — `x_source` (`file: etc/openssl.cnf`, `scan default`, `not supplied`) and `y_source`
(`action_class:config`, or the policy default with the reason) go into `rationale` beside
the numbers. The effort figures in `y_years_by_action_class` are the pack's stated
assumption and are editable there, the same standing Z has.

**X is asked for on the approval screen**, not on the intake form. That screen is the only
place the user is looking at their own file tree, and "the customer records must stay
secret for thirty years and the build cache must not" is a judgement about *these paths*
that cannot be made against an empty form. One number covers the scan; any file may be
given its own, and setting a folder assigns to every file beneath it. The lookup is by
exact path with a finding's trailing `:<line>` removed — a lifetime belongs to the file,
not to line 14 of it. A probed `host:port` has no file and takes the scan-wide value.
A missing X is still never defaulted: it routes to `verify`.
...
```json
{
  ...
  "x_source": "file: etc/openssl.cnf",
  "y_source": "action_class:config",
  "urgency_years": 1,
  "action_class": "config",
  "confidence": "high",
  "wave": "wave_1"
...
- Two key-exchange findings at the same X and Z, whose cheapest action classes differ, get different Y — and where that crosses zero, different waves
- A file given its own data lifetime on the approval screen is scored at it; every other file in the scan is scored at the scan-wide value, and each row's `x_source` says which
